...
<div class="pt-28">
    <div class="rounded-lg border bg-card text-card-foreground shadow-sm w-full max-w-md mx-auto">
        <div class="flex flex-col space-y-1.5 p-6">
            <h3 class="whitespace-nowrap text-2xl font-semibold leading-none tracking-tight">Cambiar Edad</h3>
        </div>
        <div class="p-6 space-y-4">
            <form method="POST" action="{{ route('update.age') }}">
                @csrf
                <div class="grid gap-2">
                    <div class="flex items-center justify-between">
                        <input class="h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm ring-offset-background file:border-0 file:bg-transparent file:text-sm file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2" id="age" name="age" placeholder="Ingresa tu nueva edad" type="number" />
                        @error('age')
